...
import pinar.hazard.tc_tracks as tc
import pinar.hazard.tc_tracks_synth as tc_synth
import pinar.util.coordinates
from pinar.util.constants import TC_ANDREW_FL
...
class TestDecay(unittest.TestCase):
    ...
    def test_calc_decay_no_landfall_pass(self):
        ...
        tc.track_land_params(tc_track.data[0], land_geom)
        with self.assertLogs('pinar.hazard.tc_tracks_synth', level='INFO') as cm:
...
class TestSynth(unittest.TestCase):
    ...
    def test_random_walk_decay_pass(self):
        ...
        with self.assertLogs('pinar.hazard.tc_tracks_synth', level='DEBUG') as cm0:
        ...
        tc_track = tc.TCTracks.from_processed_ibtracs_csv(TC_ANDREW_FL)
        with self.assertLogs('pinar.hazard.tc_tracks_synth', level='DEBUG') as cm:
            tc_track.calc_perturbed_trajectories(nb_synth_tracks=nb_synth_tracks, seed=25, decay=True,
