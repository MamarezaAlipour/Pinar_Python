...
from pinar.hazard.tc_tracks import TCTracks, estimate_rmw
from pinar.hazard.tc_clim_change import get_knutson_criterion, calc_scale_knutson
from pinar.hazard.centroids.centr import Centroids
from pinar.util import ureg
import pinar.util.constants as u_const
import pinar.util.coordinates as u_coord
import pinar.util.plot as u_plot
...
class TropCyclone(Hazard):
    ...
    def apply_pinar_scenario_knu(
        ...
        Returns
        -------
        haz_cc : pinar.hazard.TropCyclone
            ...
            a new instance of pinar.hazard.TropCyclone, self is not
            modified.
        """
        ...
        haz_cc.tag.description = 'pinar change scenario for year %s and RCP %s '\
        'from Knutson et al 2015.' % (str(ref_year), str(rcp_scenario))
        return haz_cc

    def set_pinar_scenario_knu(self, *args, **kwargs):
        """This function is deprecated, use TropCyclone.apply_pinar_scenario_knu instead."""
        LOGGER.warning("The use of TropCyclone.set_pinar_scenario_knu is deprecated."
                       "Use TropCyclone.apply_pinar_scenario_knu instead.")
        return self.apply_pinar_scenario_knu(*args, **kwargs)
    ...
    def video_intensity(
        ...
        **kwargs
        ...
        centroids : pinar.hazard.Centroids
    ...
    def from_single_track(
        ...
        metric : str, optional
            ...
            "geosphere" (more accurate). See `dist_approx` function in `pinar.util.coordinates`.
            Default: "equirect".
        intensity_thres : float, optional
            Wind speeds (in m/s) below this threshold are stored as 0. Default: 17.5
        max_dist_eye_km : float, optional
    ...
    def _apply_knutson_criterion(
        ...
        chg_int_freq : list(dict))
            list of criteria from pinar.hazard.tc_clim_change
        scaling_rcp_year : float
            scale parameter because of chosen year and RCP

        Returns
        -------
        tc_cc : pinar.hazard.TropCyclone
        ...
        if (tc_cc.frequency < 0).any():
            raise ValueError("The application of the given pinar scenario"
