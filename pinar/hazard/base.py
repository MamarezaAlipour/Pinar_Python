...
import pinar.util.checker as u_check
import pinar.util.dates_times as u_dt
from pinar import CONFIG
import pinar.util.hdf5_handler as u_hdf5
import pinar.util.coordinates as u_coord
from pinar.util.constants import ONE_LAT_KM, DEF_CRS, DEF_FREQ_UNIT
from pinar.util.coordinates import NEAREST_NEIGHBOR_THRESHOLD
...
class Hazard():
    ...
    def from_xarray_raster(
        ...
        *,
        ...
        meaning that the object can be used in all PINAR operations without throwing
        ...
        crs : str, optional
            ...
            to ``EPSG:4326`` (WGS 84), defined by ``pinar.util.constants.DEF_CRS``.
        ...
        :py:meth:`~Hazard.from_xarray_raster_file`
            Use this method if you want PINAR to open and read a file on disk for you.
    ...
    def from_mat(cls, file_name, description='', var_names=None):
        """Read pinar hazard generate with the MATLAB code in .mat format.
    ...
    def from_excel(cls, file_name, description='', var_names=None, haz_type=None):
        """Read pinar hazard generated with the MATLAB code in Excel format.
    ...
    def append(self, *others):
        ...
        Parameters
        ----------
        others : one or more pinar.hazard.Hazard objects
    ...
    def concat(cls, haz_list):
        ...
        Parameters
        ----------
        haz_list : list of pinar.hazard.Hazard objects
            Hazard instances of the same hazard type (subclass).

        Returns
        -------
        haz_concat : instance of pinar.hazard.Hazard
    ...
    def change_centroids(self, centroids, threshold=NEAREST_NEIGHBOR_THRESHOLD):
        ...
        threshold: int or float
            ...
            Argument is passed to pinar.util.coordinates.assign_coordinates.
            Default: 100 (km)
    ...
    def centr_exp_col(self):
        ...
        """
        from pinar.entity.exposures import INDICATOR_CENTR  # pylint: disable=import-outside-toplevel
