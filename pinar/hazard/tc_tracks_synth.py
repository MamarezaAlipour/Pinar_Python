...
import pinar.util.coordinates
import pinar.hazard.tc_tracks
...
>>> land_geom = pinar.util.coordinates.get_land_geometry(
...
def _one_rnd_walk(track, nb_synth_tracks, max_shift_ini, max_dspeed_rel, max_ddirection, rnd_vec):
    ...
    for i_ens in range(nb_synth_tracks):
        ...
        angular_dist = pinar.util.coordinates.dist_approx(i_track.lat.values[:-1, None],
        ...
        for i in range(0, len(new_lon) - 1):
            ...
            if i+2 < last_idx and (new_lat[i + 1] > 70 or new_lat[i + 1] < -70):
                ...
                ss_scale_end = pinar.hazard.tc_tracks.set_category(max_wind_end,
                ...
                cutoff_txt = (f"{i_track.attrs['name']}_gen{i_ens + 1}"
                              f" ({pinar.hazard.tc_tracks.CAT_NAMES[ss_scale_end]})")
                if ss_scale_end > 1:
        ...
        pinar.util.coordinates.lon_normalize(new_lon, center=0.0)
...
def _apply_land_decay(tracks, v_rel, p_rel, land_geom, s_rel=True,
    ...
    for track in tracks:
        if track.orig_event_flag:
            pinar.hazard.tc_tracks.track_land_params(track, land_geom)
    if check_plot:
...
def _decay_values(track, land_geom, s_rel):
    ...
    pinar.hazard.tc_tracks.track_land_params(track, land_geom)
    sea_land_idx, land_sea_idx = pinar.hazard.tc_tracks._get_landfall_idx(track)
    if sea_land_idx.size:
        for sea_land, land_sea in zip(sea_land_idx, land_sea_idx):
            ...
            ss_scale = pinar.hazard.tc_tracks.set_category(v_landfall,
...
def _decay_calc_coeff(x_val, v_lf, p_lf):
    ...
    if not scale_fill.size:
        ...
        return v_rel, p_rel
    for ss_scale, ss_name in pinar.hazard.tc_tracks.CAT_NAMES.items():
        if ss_scale not in p_rel:
            close_scale = scale_fill[np.argmin(np.abs(scale_fill - ss_scale))]
            close_name = pinar.hazard.tc_tracks.CAT_NAMES[close_scale]
...
def _check_decay_values_plot(x_val, v_lf, p_lf, v_rel, p_rel):
    ...
    for track_cat, color in zip(v_lf.keys(),
        ...
        axes[0].set_title(f'Wind, TC cat {pinar.hazard.tc_tracks.CAT_NAMES[track_cat]}')
        axes[0].plot(x_val[track_cat], v_lf[track_cat], '*', c=color,
                     label=pinar.hazard.tc_tracks.CAT_NAMES[track_cat])
        ...
        axes[1].set_title(f'Pressure, TC cat {pinar.hazard.tc_tracks.CAT_NAMES[track_cat]}')
...
def _apply_decay_coeffs(track, v_rel, p_rel, land_geom, s_rel):
    ...
    track.attrs['category'] = pinar.hazard.tc_tracks.set_category(
...
def _check_apply_decay_plot(all_tracks, syn_orig_wind, syn_orig_pres):
    ...
    scale_thresholds = pinar.hazard.tc_tracks.SAFFIR_SIM_CAT
    leg_lines = [Line2D([0], [0], color=pinar.hazard.tc_tracks.CAT_COLORS[i_col], lw=2)
                 for i_col in range(len(scale_thresholds))]
    leg_lines.append(Line2D([0], [0], color='k', lw=2))
    leg_names = [pinar.hazard.tc_tracks.CAT_NAMES[i_col]
                 for i_col in sorted(pinar.hazard.tc_tracks.CAT_NAMES.keys())]
        ...
        if sea_land_idx.size:
            for sea_land, land_sea in zip(sea_land_idx, land_sea_idx):
                ...
                graph_v_a.plot(on_land, track.max_sustained_wind[on_land],
                               'o', c=pinar.hazard.tc_tracks.CAT_COLORS[ss_scale_idx])
