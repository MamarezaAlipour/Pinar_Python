...
from pinar.hazard import Hazard, Centroids
...
class Download(Model):
    """Database entry keeping track of downloaded files from the PINAR data API"""
    url = CharField()
...
@dataclass
class DataTypeInfo():
    """data type meta data from PINAR data API."""
    data_type:str
    data_type_group:str
    status: str
    description:str
...
class DataTypeShortInfo():
    """data type name and group from PINAR data API."""
    data_type:str
    data_type_group:str
...
@dataclass
class DatasetInfo():
    """dataset data from PINAR data API."""
    uuid:str
...
def checksize(local_path, fileinfo):
    ...
    local_path : Path
        the downloaded file
    filinfo : FileInfo
        file information from PINAR data API
...
class Client():
    ...
    def list_data_type_infos(self, data_type_group=None):
        """Returns all data types from the pinar data API
    ...
    def get_data_type_info(self, data_type):
        """Returns the metadata of the data type with the given name from the pinar data API.
    ...
    def get_hazard(self, hazard_type, name=None, version=None, properties=None,
        ...
        hdf5 files and turns them into a pinar.hazard.Hazard object.

        Parameters
        ----------
        hazard_type : str
            Type of pinar hazard.
        name : str, optional
            the name of the dataset
        version : str, optional
            the version of the dataset
        ...
        dump_dir : str, optional
            ...
            pinar.conf, i.g. ~/pinar/data), the eventual target directory is organized into
        ...
        if not hazard_type in HAZ_TYPES:
            raise ValueError("Valid hazard types are a subset of PINAR hazard types."
    ...
    def to_hazard(self, dataset, dump_dir=SYSTEM_DIR):
        ...
        concatenates them into a single pinar.Hazard object.

        Parameters
        ----------
        dataset : DatasetInfo
            Dataset to download and read into pinar.Hazard object.
        dump_dir : str, optional
            Directory where the files should be downoladed. Default: SYSTEM_DIR (as configured in
            pinar.conf, i.g. ~/pinar/data).
    ...
    def get_exposures(self, exposures_type, name=None, version=None, properties=None,
        ...
        hdf5 files and turns them into a pinar.entity.exposures.Exposures object.

        Parameters
        ----------
        exposures_type : str
            Type of pinar exposures.
        name : str, optional
            the name of the dataset
        version : str, optional
            the version of the dataset
        ...
        Returns
        -------
        pinar.entity.exposures.Exposures
            The combined exposures object
        """
        if not exposures_type in EXP_TYPES:
            raise ValueError("Valid exposures types are a subset of PINAR exposures types."
    ...
    def to_exposures(self, dataset, dump_dir=SYSTEM_DIR):
        ...
        concatenates them into a single pinar.Exposures object.

        Parameters
        ----------
        dataset : DatasetInfo
            Dataset to download and read into pinar.Exposures objects.
        dump_dir : str, optional
            Directory where the files should be downoladed. Default: SYSTEM_DIR (as configured in
            pinar.conf, i.g. ~/pinar/data).
            If the directory is the SYSTEM_DIR, the eventual target directory is organized into
    ...
    def get_centroids(self, res_arcsec_land=150, res_arcsec_ocean=1800,
        ...
        Returns
        -------
        pinar.hazard.centroids.Centroids
    ...
    def get_property_values(dataset_infos, known_property_values=None,
        ...
        exclude_properties: list of str, optional
            ...
            Default: 'creation_date', 'pinar_version'
        ...
        if exclude_properties is None:
            exclude_properties = ['date_creation', 'pinar_version']
