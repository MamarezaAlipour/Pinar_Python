...
LOGGER = logging.getLogger('pinar')
...
class Config():
    ...
    Like this it's possible to refer to e.g. test files in the pinar sources.
    """
...
CONFIG_NAME = 'pinar.conf'
CONFIG = Config.from_dict(_fetch_conf([
    Path(SOURCE_DIR, 'pinar', 'conf'),  # default config from the pinar repository
    Path(Path.home(), 'pinar', 'conf'),  # ~/pinar/conf directory
