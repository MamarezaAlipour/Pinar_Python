...
from pinar.util.finance import gdp
from pinar.util.constants import DEF_CRS
import pinar.util.coordinates as u_coord
from pinar.engine import Impact
from pinar.entity.tag import Tag
from pinar.hazard.tag import Tag as TagHaz
...
def clean_emdat_df(emdat_file, countries=None, hazard=None, year_range=None,
    ...
    hazard : list or str
        ...
        Tsunami, etc.;
        OR PINAR hazard type abbreviations, e.g. TC, BF, etc.
...
def emdat_to_impact(emdat_file_csv, hazard_type_pinar, year_range=None, countries=None,
    ...
    Returns
    -------
    impact_instance : pinar.engine.Impact
        impact object of same format as output from PINAR
        impact computation.
    ...
    if not hazard_type_emdat:
        hazard_type_emdat = [hazard_type_pinar]
    if reference_year == 0:
        reference_year = None
    ...
    impact_instance.tag['haz'] = TagHaz(haz_type=hazard_type_pinar,
