...
from pinar.engine.impact_calc import ImpactCalc
from pinar.engine import Impact, ImpactFreqCurve
...
class CostBenefit():
    ...
    future_year : int
        future year
    tot_pinar_risk : float
        total pinar risk without measures
    unit : str
    ...
    def __init__(
        ...
        self.tot_pinar_risk = tot_pinar_risk
        self.unit = unit
    ...
    def calc(self, hazard, entity, haz_future=None, ent_future=None, future_year=None,
        ...
        hazard : pinar.Hazard
        entity : pinar.entity
        haz_future : pinar.Hazard, optional
    ...
    def combine_measures(self, in_meas_names, new_name, new_color, disc_rates,
        ...
        new_cb = CostBenefit(
            ...
            unit=self.unit,
            tot_pinar_risk=self.tot_pinar_risk,
    ...
    def plot_cost_benefit(self, cb_list=None, axis=None, **kwargs):
        ...
        norm_fact, norm_name = _norm_values(self.tot_pinar_risk + 0.01)
        ...
        if abs(text_pos - self.tot_pinar_risk / norm_fact) > 1:
            axis.scatter(self.tot_pinar_risk / norm_fact, 0, c='r', zorder=200, clip_on=False)
            axis.text(self.tot_pinar_risk / norm_fact, 0, '  Tot risk',
                      horizontalalignment='center', verticalalignment='bottom', rotation=90,
                      fontsize=12, color='r')

        axis.set_xlim(0, max(self.tot_pinar_risk / norm_fact,
    ...
    def plot_waterfall(hazard, entity, haz_future, ent_future,
        ...
        # socioecon + cc
        LOGGER.info('Risk with development and pinar change at {:d}: {:.3e}'.
    ...
    def plot_arrow_averted(self, axis, in_meas_names=None, accumulate=False, combine=False,
        ...
        if accumulate:
            ...
            norm_fact = self.tot_pinar_risk / bars[3].get_height()
        else:
    ...
    def plot_waterfall_accumulated(self, hazard, entity, ent_future,
        ...
        LOGGER.info('Total risk with development and pinar change at {:d}: {:.3e}'.
    ...
    def _calc_impact_measures(self, hazard, exposures, meas_set, imp_fun_set,
        ...
        ----------
        hazard : pinar.Hazard
        exposures : pinar.entity.Exposures
        meas_set : pinar.MeasureSet
            set of measures.
    ...
    def _calc_cost_benefit(self, disc_rates, imp_time_depen=None):
        ...
        # discounted cost benefit for each measure and total pinar risk
        for meas_name, meas_val in self.imp_meas_future.items():
            if meas_name == NO_MEASURE:
                # npv of the full unaverted damages
                if self.imp_meas_present:
                    self.tot_pinar_risk = self._npv_unaverted_impact(
    ...
    def _npv_unaverted_impact(self, risk_future, disc_rates, time_dep,
        ...
        """
        if risk_present:
            tot_pinar_risk = risk_present + (risk_future - risk_present) * time_dep
            tot_pinar_risk = disc_rates.net_present_value(self.present_year,
                                                            self.future_year,
                                                            tot_pinar_risk)
        else:
            tot_pinar_risk = disc_rates.net_present_value(self.present_year,
    ...
    def _print_results(self):
        ...
        table = []
        table.append(['Total pinar risk:',
                      self.tot_pinar_risk / norm_fact, norm_name])
    ...
    def _plot_list_cost_ben(cb_list, axis=None, **kwargs):
        ...
        if 'alpha' not in kwargs:
            kwargs['alpha'] = 0.5
        norm_fact = [_norm_values(cb_res.tot_pinar_risk)[0] for cb_res in cb_list]
        ...
        for i_cb, cb_res in enumerate(cb_list):
            ...
            xy_lim[0] = max(xy_lim[0],
                            max(int(cb_res.tot_pinar_risk / norm_fact),
