...
from pinar.entity import ImpactFuncSet, ImpfTropCyclone, impact_funcs
from pinar.engine.impact_data import emdat_impact_yearlysum, emdat_impact_event
    ...
    if yearly_impact:  # impact per year
        ...
        if df_out.empty | df_out.index.shape[0] == 1:
            for cnt_, year in enumerate(np.sort(list((iys.keys())))):
                ...
                if year in iys:
                    df_out.loc[cnt_, 'impact_PINAR'] = iys[year]
                else:
                    df_out.loc[cnt_, 'impact_PINAR'] = 0.0
            ...
            for cnt_, year in years_in_common.iteritems():
                df_out.loc[df_out['year'] == year, 'impact_PINAR'] = iys[year]
        ...
        if df_out.empty | df_out.index.shape[0] == 1:
            for cnt_, impact in enumerate(impacts.at_event):
                if cnt_ > 0:
                    df_out.loc[cnt_] = df_out.loc[0]  # copy info from first row
                df_out.loc[cnt_, 'impact_PINAR'] = impact
            ...
            for cnt_, (impact, ind) in enumerate(zip(impacts.at_event, df_out.index)):
                df_out.loc[ind, 'impact_PINAR'] = impact
...
def calib_cost_calc(df_out, cost_function):
    """calculate the cost function of the modelled impact impact_PINAR and
    ...
    if cost_function == 'R2':
        cost = np.sum((pd.to_numeric(df_out['impact_scaled']) -
                       pd.to_numeric(df_out['impact_PINAR']))**2)
    elif cost_function == 'logR2':
        impact1 = pd.to_numeric(df_out['impact_scaled'])
        impact1[impact1 <= 0] = 1
        impact2 = pd.to_numeric(df_out['impact_PINAR'])
        impact2[impact2 <= 0] = 1
