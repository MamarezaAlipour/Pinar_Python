...
from pinar.hazard.base import Hazard, Centroids
from pinar.util.constants import ENT_TEMPLATE_XLS, ONE_LAT_KM, DEF_CRS, HAZ_DEMO_FL
...
class TestGeoDFFuncs(unittest.TestCase):
    ...
    def test_error_on_access_item(self):
        """Test error output when trying to access items as in PINAR 1.x"""
        expo = good_exposures()
        with self.assertRaises(TypeError) as err:
            expo['value'] = 3
        self.assertIn("PINAR 2", str(err.exception))
