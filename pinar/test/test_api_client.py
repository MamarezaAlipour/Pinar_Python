...
from pinar.util.api_client import Client, DataTypeShortInfo, Download
...
class TestClient(unittest.TestCase):
    ...
    def test_dataset_offline(self):
        ...
        with self.assertLogs('pinar.util.api_client', level='WARNING') as cm:
        ...
        with self.assertRaises(ValueError) as cm:
            client.get_exposures(exposures_type='river_flood',
                                 properties={'country_iso3alpha': 'AUT',
                                             'fin_mode': 'pop', 'exponents': '(0,1)'},
                                 dump_dir=DATA_DIR)
        self.assertIn('Valid exposures types are a subset of PINAR exposures types. Currently',
                      str(cm.exception))
    ...
    def test_get_hazard(self):
        ...
        hazard = client.get_hazard(hazard_type='river_flood',
                                   properties={'country_name': 'Austria',
                                               'year_range': '2010_2030', 'pinar_scenario': 'rcp26'},
                                   version='v1',
                                   dump_dir=DATA_DIR)
        ...
        with self.assertRaises(ValueError) as cm:
            client.get_hazard(hazard_type='litpop',
                              properties={'country_name': 'Austria',
                                          'year_range': '2010_2030', 'pinar_scenario': 'rcp26'},
                              dump_dir=DATA_DIR)
        self.assertIn('Valid hazard types are a subset of PINAR hazard types. Currently',
                      str(cm.exception))

        with self.assertRaises(Client.AmbiguousResult) as cm:
            client.get_hazard(hazard_type='river_flood',
                              properties={'country_name': ['Switzerland', 'Austria'],
                                          'year_range': '2010_2030', 'pinar_scenario': ['rcp26', 'rcp85']},
                              dump_dir=DATA_DIR)
